...
/*
    ...
    the final stbl / collateral price. In particular, we need to determine

      a. `gap`, the collateral shortfall per collateral type by
         considering under-collateralised CDPs.

      b. `debt`, the outstanding stbl supply after including system
         surplus / deficit
    ...
    We determine (b) by processing ongoing stbl generating processes,
    i.e. auctions. We need to ensure that auctions will not generate any
    further stbl income. In the two-way auction model this occurs when
    ...
    4.  i) `wait`: set the cooldown period to be at least as long as the
           ...
           auction dynamics due to the now varying price of stbl.

       ii) `skip`: cancel all ongoing auctions and seize the collateral.

           This allows for faster processing at the expense of more
           processing calls. This option allows stbl holders to retrieve
           their collateral faster.

           `skip(ilk, id)`:
            - cancel individual flip auctions in the `tend` (forward) phase
            - retrieves collateral and returns stbl to bidder
    ...
    6. `thaw()`:
       ...
       - fixes the total outstanding supply of stbl
    ...
    type and stbl holders can now turn their stbl into collateral. Each
    unit stbl can claim a fixed basket of collateral.

    Stbl holders must first `pack` some stbl into a `bag`. Once packed,
    stbl cannot be unpacked and is not transferrable. More stbl can be
    added to a bag later.

    8. `pack(wad)`:
        - put some stbl into a bag in preparation for `cash`

    Finally, collateral can be obtained with `cash`. The bigger the bag,
    the more collateral can be released.

    9. `cash(ilk, wad)`:
        - exchange some stbl from your bag for gems from a specific ilk
...
contract End is LibNote {
    ...
    uint256  public debt;  // Total outstanding stbl following processing [rad]
    ...
    function thaw() external note {
        ...
        require(vat.stbl(address(vow)) == 0, "End/surplus-not-zero");
